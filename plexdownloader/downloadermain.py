...
from plexdownloader.modules import *
from plexdownloader.embeds import *
...
class Downloader(commands.Cog):
    ...
    async def movie(self, ctx, *, args):
        ...
        if ctx.message.channel.id == discordChannelId:
            ...
            try:
                option = await self.bot.wait_for('message', timeout=45, check=check(ctx.author))
            ...
            try:
                ...
                embed = torrentembed(downloadname, downloadpage, downloadsize, seeders, leechers, movieposters[optionchoosen], optionchoosen, ctx)
            ...
            if Worked:
                ...
                self.bot.loop.create_task(update(message.id, downloadlink, ctx))
    ...
    async def show(self, ctx, *, args):
        ...
        if ctx.message.channel.id == discordChannelId:
            ...
            try:
                Worked = startDownload(downloadlink, downloadcategory)
                embed = torrentembed(downloadname, downloadpage, downloadsize, seeders, leechers, seriesposters[optionchoosen], optionchoosen, ctx)
                message = await ctx.send(embed=embed)
